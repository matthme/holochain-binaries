...
getAndDo(holochainReleasesOptions, (data) => {
  ...
  getAndDo(binariesTagsOptions, (data) => {
    ...
    // Check for holochain 0.6 releases that have not yet a tag in the holochain-binaries repo
    ...
    const unbuilt05Releases = unbuiltReleases.filter((tagName) => tagName.startsWith('holochain-0.6'));
